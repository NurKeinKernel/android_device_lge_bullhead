...
extract "$MY_DIR/du-proprietary-blobs.txt" "$SRC"
...
if [ "$SRC" != "adb" ]; then
  for file in $(egrep -v '(^#|^$)' "$MY_DIR"/du-proprietary-blobs-vendorimg.txt); do
...
extract "$MY_DIR/du-proprietary-blobs-vendorimg.txt" "$SRC"
